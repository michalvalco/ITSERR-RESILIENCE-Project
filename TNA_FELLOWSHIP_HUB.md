...
**Date:** Feb 14, 2026 (Day 5 of 14) — evening session
**Active work:** Cross-document consistency audit across all four pipeline documents (workflow_diagram.md, pipeline_overview.mermaid, pipeline_stage4_deep_dive.mermaid, pipeline_technical_reference.md). Found and fixed 6 inconsistencies: (1) FACTUAL criteria AND→OR logic alignment with actual code; (2) cas_to_bioes.py stale "hardcodes AN" claim → confirmed multi-type support via get_annotation_type_label(); (3) pipeline architecture sequential→parallel (each layer reads ZIP independently); (4) test count 393→413; (5) Stage 5 missing working deliverables (build_corpus_json.py, Corpus Browser); (6) Stage 5 data flow current vs. future path disambiguation. All 245 tests passing after changes. Drafted email to Arianna and Marcello introducing all deliverables (workflow, mermaids, pipeline reference, Corpus Browser prototype at https://michalvalco.github.io/ITSERR-RESILIENCE-Project/prototype/). Email frames work as Phase 1 of wider project.
**Next:** (1) ✉️ Send email to Arianna/Marcello with attachments. (2) Full revision pass on working paper. (3) Prepare consortium presentation (Feb 25). (4) Zero-shot test on Stöckel sample.
...
| 2 | **GNORM/Stöckel Workflow** | `01_research/workflow_diagram.md` | Workflow diagram complete (7 stages). Cross-document consistency audit done — all 4 pipeline docs aligned with codebase. Email to Arianna/Marcello drafted. Next: send email, zero-shot test, entity schema validation |
...
| 4 | **Prototype** | `03_prototype/` + `stockel_annotation/PROGRESS.md` | OCR pipeline scripts (`ocr_processor`, `extract_alto`, `normalize_text`), corpus data generator (`build_corpus_json`), ML pipeline (`cas_to_bioes`, `zero_shot_crf`), Corpus Browser with dashboard and 31 detected references. Abbreviation expansion provenance logging for Stage 4 Layer 2. 414 tests passing across 11 suites (see `PROGRESS.md` for authoritative count). |
...
| **Progress Tracker** | `ITSERR repo: 03_prototype/stockel_annotation/PROGRESS.md` | **Definitive reference for what's built and tested.** All scripts, test results (414 tests across 11 suites), OCR quality assessment, data flow, dependencies. Consult when asking "what do we have?" or "does X work?" |
...
| 14 Feb 2026 | Pipeline documentation review for Palermo submission | Cross-referenced workflow_diagram.md + both mermaids against actual code. Three critical fixes: (1) cas_to_bioes.py now explicitly marked as "adapted" version (original CIC hardcodes AN); (2) FACTUAL threshold in mermaids corrected to show both biblical >= 0.85 and confessional >= 0.80; (3) test counts unified with PROGRESS.md as authoritative source. Ready for Arianna/Marcello review. |
...
| 14 Feb 2026 | Cross-document consistency audit for Palermo submission | Systematic integrity check across workflow_diagram.md, both mermaid diagrams, and pipeline_technical_reference.md. Six fixes: FACTUAL criteria OR-logic, cas_to_bioes.py multi-type confirmation, parallel architecture representation, test counts, Stage 5 working deliverables, dual-path data flow. All four documents now tell consistent story aligned with actual codebase. 245 tests passing. |
| 14 Feb 2026 | Email to Arianna/Marcello drafted | Introduces all deliverables: workflow diagram, mermaid diagrams, pipeline technical reference, Corpus Browser prototype (https://michalvalco.github.io/ITSERR-RESILIENCE-Project/prototype/). Frames as Phase 1 of wider project ("Designing Ethically-Grounded AI Agents for Religious Studies Research"). Tone: measured and consultative. Mentions AI coding assistant usage (Claude, Copilot, Gemini, Codex). Three specific questions for their expertise. Pending: send with attachments. |
